...
public class DisplayElement extends Dimension {
    ...
    DisplayElement(JsonHolder object) {
        ...
        scale = object.optDouble(SCALESTRING);
        ...
        color = data.optInt(COLORSTRING);
        chroma = data.optBoolean("chroma");
        rightSided = data.optBoolean("right_side");
        rgb = data.optBoolean("rgb");
        color_pallet = data.optBoolean(COLORPALLETSTRING);
        static_chroma = data.optBoolean("static_chroma");
        recalculateColor();
    }

    public static DisplayElement blank() {
        return new DisplayElement(new JsonHolder().put("x", .5).put("y", .5).put(SCALESTRING, 1).put(COLORSTRING, Color.WHITE.getRGB()).put(COLORPALLETSTRING, true));
    }
    ...
    public void setColorPallet(boolean state) {
        color_pallet = state;
        data.put(COLORPALLETSTRING, state);
    }
